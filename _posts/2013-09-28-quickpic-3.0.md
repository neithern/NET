...
New view option for 4.1+: auto hide soft keys, immersive mode for KitKat 4.4
Double touch drag up or down to zoom like Chrome
Drag to seek or volume for video player
Sort by EXIF taken date for single folder
Improve performance of sort by numerical(logical)
Fix some bugs
...
新查看选项：自动隐藏虚拟按键（4.1及以上，KitKat 4.4 沉浸式全屏）
双击后上下滑动缩放图像（类似Chrome）
视频播放时滑动屏幕可以快速定位或调节音量
单个文件夹可以按EXIF拍摄日期排序
优化按数字式（逻辑）排序的性能
